...
class EvalModel:
    ...
    def Sampling_from_latent_space(self, VAE):
        VAE.eval()
        ...
        plt.savefig(cfg.DIR_out + 'check/' +'VAE_sample_from_latent_space.png')
        plt.close()
        return 1

    def Reconstruction(self, VAE, all_dataset, L_idx):
        ...
        for j in range(len(L_idx)):
            ...
            recons = VAE.generate(tensor_img.to(device))
        ...
        plt.savefig(cfg.DIR_out + 'check/' +'VAE_imge_reconstruction.png')
        plt.close()
        return 1


    def Interpolation(self, VAE, all_dataset, idx_img1, idx_img2):
        ...
        granularity = 10
        interpolation = VAE.interpolate(starting_inputs=starting_inputs, ending_inputs=ending_inputs, device=device,
        ...
        plt.tight_layout(pad=0.)
        plt.savefig(cfg.DIR_out + 'check/' + 'VAE_img_interpolation_sample.png')
        plt.close()

    def anomaly_detection(self, model, all_dataset):
        n_sample = 5   # number of sample, n_sample normals & n_sample abnormal images.
        ...
        percentile_thr = 99
        threshold = np.percentile(recon_errors, percentile_thr)
        anomalies = recon_errors > threshold

        error_and_img_idx = [(recon_errors[i], L_idx[i]) for i, v in enumerate(recon_errors)]
        top_big_n = heapq.nlargest(2*n_sample, error_and_img_idx)
        anomalies_idx = [int(index) for value, index in top_big_n]
        ...
        plt.title("Anomaly Detection using VAE (" + str(percentile_thr) + " % )")
        plt.savefig(cfg.DIR_out + 'check/' + 'VAE_anomaly_detection_hist.png')
        plt.close()
        ...
        plt.tight_layout(pad=0.)
        plt.savefig(cfg.DIR_out + 'check/' +'VAE_anomaly_detection_samples.png')
        plt.close()
        return 1