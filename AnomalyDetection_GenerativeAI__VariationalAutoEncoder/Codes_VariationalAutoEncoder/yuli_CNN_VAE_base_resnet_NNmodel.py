...
class ConvEncoder(nn.Module):
    ...
    def forward(self, x):    #, init_hidden):
        # input x.shape = [B,C,H,W] = [Batch_size, Channel, Height, Width]   #in: [B, 1, 48, 48]
        x = self.InitialLayer(x)  # initial conv. with bigger kernel  , out: [B, 64, 48/(2**1), 48/(2**1)]
        x = self.maxpool(x)   # de-noise  # out: [B, 64, 48/(2**2), 48/(2**2)]
        ...
        return [mu, log_var]  # return x  # F.log_softmax(x, dim=1) # CrossEntropyLoss already have nn.LogSoftmax on it.
...
class VariationalAutoEncoder(nn.Module):
    ...
    def reparameterize(self, mu, log_var):
        ...
        eps = torch.randn_like(std)  # Random noise (0 mean & 1 var).
    ...
    def loss_function(self, recons, input, mu, log_var):
        ...
        # recons_loss = nn.functional.mse_loss(recons, input, reduction="none").sum(dim=[1,2,3])
            # recons.reshape(recons.shape[0], -1), input.reshape(input.shape[0], -1), reduction="none").sum(dim=-1)
        kld_loss = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp(), dim=-1)  # or dim=[1]# TODO: check this
    ...
    def interpolate(self, starting_inputs, ending_inputs, device, granularity=10):
        ...
        starting_z = self.reparameterize(mu, log_var)  # size [B, latent_dims], B=1 here

        mu, log_var = self.encoder(ending_inputs.to(device))
        ending_z = self.reparameterize(mu, log_var)  # size [B, latent_dims]
