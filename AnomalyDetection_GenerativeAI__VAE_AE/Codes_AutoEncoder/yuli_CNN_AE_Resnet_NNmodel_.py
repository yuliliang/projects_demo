...
class Encoder_ResNetBlock(nn.Module):
    ...
    def __init__( self, flag: str, h_dim_in: int, h_dim_out: int) -> None:
        ...
        elif self.flag == 'Residual_Stable':   # same input and output channel
            self.ConvLayer = nn.Sequential(
...
class ConvEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.InitialLayer(x)  # initial conv. with bigger kernel
        x = self.maxpool(x)   # de-noise
...
class AutoEncoder(nn.Module):
    ...
    def interpolate(self, starting_inputs, ending_inputs, device, granularity=10):
        """
        Interpolating in the latent space of the autoencoder from starting inputs to ending inputs.
        Returns the interpolation trajectories.
        """
        ...
        decoded_line = self.decoder(intep_line).reshape(
            (starting_inputs.shape[0], t.shape[0])
