...
def record_file_names(folder_path, output_file):
    """
    Goal: Search the file in the input folder and records the file names

    Args:
        folder_path: The folder with input image files
        output_file: A *.xlsx that saved the image files names in folder_path.
    """
    L_img_file = []
    for root, dirs, files in os.walk(folder_path):
        for filename in files:
            if filename !='.DS_Store':
                L_img_file.append(os.path.join(root, filename))
    # pd.DataFrame(data=L_img_file, columns=['FName']).to_excel(output_file)
    df = pd.DataFrame(data=L_img_file, columns=['FName'])
    df['article_id'] = df['FName'].apply(lambda x: x.split("/")[-1][1:-4]).astype(int)
